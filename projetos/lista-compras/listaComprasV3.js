...
const verifyUser = (ctx, next) => {
    const isSameUserIdMsg = ctx.update.message && 
        ctx.update.message.from.id === env.userID
    const isSameUserIdAction = ctx.update.callback_query && 
        ctx.update.callback_query.from.id === env.userID
        
    if (isSameUserIdMsg || isSameUserIdAction) {
        next()
    } else {
...
bot.on('text', verifyUser, inProcessing, ctx => {
...
bot.action(/delete (.+)/, verifyUser, async ctx => {
